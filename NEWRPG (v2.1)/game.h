...
typedef struct {
    bool discovered[SPELL_COUNT]; float notificationTimer; char notificationText[64]; float scrollY; 
} Compendium;
...
// --- GRASS ---
typedef struct {
    Vector2 position; float angle; float targetAngle; float stiffness; float height; Color color;
} Grass;
